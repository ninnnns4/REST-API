...
userRouter.post("/login", async (req : Request, res : Response) => {
    try {
        ...
        if (!email || !password){
            return res.status(StatusCodes.BAD_REQUEST).json({error : "Pleae provide all the required parameters.."})
        }
        const user =  await database.findByEmail(email)

        if (!user) {
            return res.status(StatusCodes.NOT_FOUND).json({error : "Incorrect Password!"})
        }
...
userRouter.put('/user/:id', async (req : Request, res : Response) =>{
    try {
        const {username,email,password} = req.body
        const getUsers = await database.FindOne(req.params.id)
